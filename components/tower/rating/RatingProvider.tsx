...
const RatingFormProvider = async ({ tower }: { tower: Tower }) => {
    let [userRating, towerRatings] = await Promise.all([
        getUserRating(tower.id),
        getTowerRatings(tower.id),
    ]);
    const users = await Promise.all(towerRatings.map((rating) => getUserById(rating.user_id)));
    ...
    if (userRating) {
        userRating.user = await getUserById(userRating.user_id);
        towerRatings = towerRatings.filter((r) => r.user_id !== userRating.user_id);
        const promises = towerRatings.map((rating) => getUserById(rating.user_id));
    ...
    return (
        <RatingForm
            ...
            ratings={towerRatings}
            users={users}
            updateTowerRating={updateTowerRating}
        />
    );
};
